...
func main() {
	...
	handler := middleware.Logging(mux)         //Первая обертка хэндлеров. Добавляет функционал логирования выполнения запросов: метод запроса, URI, затраченное время на выполнение запроса
	handler = middleware.PanicHandler(handler) //Вторая обертка хэндлеров. Добавляет функционал обработки runtime ошибок. Вместо закрытия клиентского соединения, ему будет возвращен статус. Соединение не будет закрыто, т.к.
	//при обработке ошибок использовалась функция Recovery.

	fmt.Println("Starting server on localhost:8088")
	log.Fatal(http.ListenAndServe("localhost:8088", handler))

	//Создание т запуск сервера принимающего запросы только по HTTPS протоколу.
	/*svr := &http.Server{
		Addr:    ":8080",
		Handler: mux,
